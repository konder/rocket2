...
Usage:
    ...
    # Locate bottlenecks (per-episode timing: warmup, goal_gen, policy vs env/step)
    python benchmark/benchmark_eval.py \\
        --task-file eval/eval_tasks_paper.yaml \\
...
from eval.goal_generator import (
    MockGoalGenerator,
    GoalGeneratorBase,
    MolmoGoalGenerator,
    GroundingDinoGoalGenerator,
)
from eval.rocket2_agent import Rocket2Agent
...
def create_env_from_paper_task(
    ...
    return MinecraftSim(callbacks=callbacks, num_empty_frames=num_empty_frames)


def create_env_from_full_task(
    task: Dict,
    num_empty_frames: int = 20,
) -> MinecraftSim:
...
def run_single_episode(
    ...
    When profile=True, result includes "timing" dict for bottleneck analysis.
    """
    agent.reset()
    ...
    if profile:
        t_goal_first = time.perf_counter() - t0
    ...
    for step in range(max_steps):
        try:
            ...
            if not goal_found and step > 0 and step % retry_interval == 0:
                ...
                if profile:
                    t_goal_retry_total += time.perf_counter() - t0_retry
            ...
            if profile:
                t_policy_total += time.perf_counter() - t0_p
            if profile:
                t0_e = time.perf_counter()
            obs, reward, terminated, truncated, info = env.step(action)
            if profile:
                t_env_total += time.perf_counter() - t0_e
                num_steps_done += 1
    ...
    if profile:
        n = max(num_steps_done, 1)
        result["timing"] = {
            "warmup_sec": round(t_warmup, 3),
            "goal_first_sec": round(t_goal_first, 3),
            "goal_retry_sec": round(t_goal_retry_total, 3),
            "policy_total_sec": round(t_policy_total, 3),
            "env_total_sec": round(t_env_total, 3),
            "steps": n,
            "policy_ms_per_step": round(1000 * t_policy_total / n, 2),
            "env_ms_per_step": round(1000 * t_env_total / n, 2),
        }
...
def _print_timing_hints(timing: Dict[str, Any]) -> None:
    """Print one-line bottleneck hint and optional adjustment suggestions."""
    warmup = timing.get("warmup_sec", 0)
    goal_first = timing.get("goal_first_sec", 0)
    policy_ms = timing.get("policy_ms_per_step", 0)
    env_ms = timing.get("env_ms_per_step", 0)
    steps = timing.get("steps", 1)
    total_step_ms = policy_ms + env_ms
    if total_step_ms <= 0:
        return
    parts = []
    if warmup > 2.0:
        parts.append(f"warmup={warmup:.1f}s")
    if goal_first > 1.0:
        parts.append(f"goal_first={goal_first:.1f}s")
    if env_ms > policy_ms * 1.5:
        parts.append(f"env/step={env_ms:.1f}ms (sim dominant)")
    elif policy_ms > env_ms * 1.5:
        parts.append(f"policy/step={policy_ms:.1f}ms (inference dominant)")
    else:
        parts.append(f"policy={policy_ms:.1f}ms env={env_ms:.1f}ms/step")
    if parts:
        print(f"    [profile] {' | '.join(parts)}")
    # One-line adjustment hint
    if env_ms > policy_ms * 1.5 and env_ms > 20:
        print("    [profile] → Sim bottleneck: use headless/xvfb, lower render res, or fewer steps.")
    elif policy_ms > env_ms * 1.5 and policy_ms > 30:
        print("    [profile] → Policy bottleneck: try FP16, torch.compile, or smaller model.")
...
def run_task_evaluation(
    ...
    When reuse_env=True (default), one env is created per task and reset between
    episodes to avoid repeated Minecraft process startup (large warmup cost).
    We always use full env.reset() (world/blocks/entities restored). Do not use
    MineStudio's FastResetCallback here: it only teleports the agent and does
    not reset blocks, so mine/hunt tasks would have no target on episode 2+.
    """
    task_name = task["name"]
    ...
    if reuse_env:
        print(f"  Env: reuse one env per task (reset between episodes)")
    print(f"{'='*60}")
    ...
    if reuse_env:
        try:
            if is_paper_task:
                env = create_env_from_paper_task(
                    task, env_conf_dir, num_empty_frames=num_empty_frames
                )
            else:
                env = create_env_from_full_task(task, num_empty_frames=num_empty_frames)
        except Exception:
            traceback.print_exc()
            for _ in range(num_episodes):
                episode_results.append({"success": False, "error": "env_creation_failed"})
            print("  FAIL (env creation failed for all episodes)")
            return {
                "task_name": task_name,
    ...
    for ep in range(num_episodes):
        ...
        if not reuse_env:
            try:
                if is_paper_task:
                    ...
                    env = create_env_from_full_task(
                        task, num_empty_frames=num_empty_frames
                    )
            except Exception:
                traceback.print_exc()
                episode_results.append({"success": False, "error": "env_creation_failed"})
        ...
        try:
            ...
            if profile and result.get("timing"):
                _print_timing_hints(result["timing"])
    ...
    if reuse_env and env is not None:
        try:
            env.close()
        except Exception:
            pass
...
def main():
    ...
    out_group.add_argument("--profile", action="store_true",
                           help="Print per-episode timing (warmup, goal_gen, policy vs env/step) to locate bottlenecks")
    out_group.add_argument("--no-reuse-env", action="store_true",
                           help="Create a new Minecraft env per episode (slower; default is reuse env and reset)")
    out_group.add_argument("--num-empty-frames", type=int, default=20,
                           help="Frames to skip after reset to avoid black screen (default: 20; try 10 or 5 to speed up reset)")
    ...
    for task_idx, task in enumerate(tasks, 1):
        ...
        result = run_task_evaluation(
            ...
            reuse_env=not args.no_reuse_env,
            num_empty_frames=args.num_empty_frames,
        )
