...
def get_connected_components(mask):
    ...
    try:
        ...
        return _get_connected_components_pure_python(mask)


def _get_connected_components_pure_python(mask):
    """Pure-Python fallback using cv2.connectedComponentsWithStats (CPU only)."""
    import cv2
    import numpy as np
    ...
    for i in range(N):
        ...
        # stats[:, cv2.CC_STAT_AREA] gives area of each component (including bg=0)
        areas = stats[:, cv2.CC_STAT_AREA]
        # Map each pixel's label to its component area; background (label 0) gets area 0
