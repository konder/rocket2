...
def json_action_to_env_action(json_action, predicted=True):
    """
    ...
    if predicted:
        env_action["camera"] = np.array([0, 0]) # seems like a bug in the original code, incorrectly set to integer, but have to keep this for the action-prediction model to work
    else:
